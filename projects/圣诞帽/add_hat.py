import cv2
import dlib  # pip install dlib-19.8.1-cp36-cp36m-win_amd64.whl
...
def add_hat(img,hat_img):
    ...
    # cv2.imwrite("hat_alpha.jpg",alpha)
    ...
    # 如果检测到人脸,如果检测到多脸利用循环对每个人脸都进行一遍操作
    if len(dets)>0:  
        for d in dets:
            ...
            #  根据人脸大小调整帽子大小, round()四舍五入,重新调整的帽子大小为人脸宽高乘以因子数
            factor = 1.5
            ...
            # 用alpha通道作为mask
            mask = cv2.resize(alpha, (resized_hat_w,resized_hat_h))
            ...
            alpha = mask_inv.astype(float)/255  # 白底黑帽
            ...
            bg = bg.astype('uint8')
            # cv2.imshow("bg", bg)
            # 提取帽子区域，bitwise_and是对二进制数据进行“与”操作，即对图像（灰度图像或彩色图像均可）每个像素值进行二进制“与”操作
            hat = cv2.bitwise_and(resized_hat,resized_hat, mask = mask)
            cv2.imwrite("hat.jpg",hat)
            # 两个ROI区域相加，cv2.add() 将两幅图像进行加法运算
...
# 读取头像图,这里是输入图像
img = cv2.imread("mayun.jpg")
output = add_hat(img,hat_img)

# 展示效果
cv2.imshow("output",output )  
cv2.waitKey(0)  
cv2.imwrite("./out/output.jpg",output)
